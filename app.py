...
BREEDS = {
    ...
    "бомбардир":  {"title": "Бомбардир",  "tier": "B", "hp": 13, "dmg": (3, 6), "crit": 0.18, "dodge": 0.09},
    "рой":        {"title": "Рой",        "tier": "B", "hp": 12, "dmg": (3, 5), "crit": 0.16, "dodge": 0.10},
    ...
    "сирена":     {"title": "Сирена",     "tier": "A", "hp": 13, "dmg": (3, 5), "crit": 0.16, "dodge": 0.14},
    "гидра":      {"title": "Гидра",      "tier": "A", "hp": 14, "dmg": (2, 5), "crit": 0.14, "dodge": 0.08},
...
async def promo_cmd(msg: types.Message):
    ...
    if code == "v1gazboss":
        ...
        p["coins"] += 100000
        p["used_promo"] = True
        save_player(msg.from_user.id, p)
        await msg.answer("🎁 Промокод активирован! Ты получил **100 000 монет**!", parse_mode="Markdown")
    else:
